...
def process_color_analysis(image_bytes):
    ...
    if rb_diff > 65 and bg_ratio < 0.85:
        undertone = "Warm"
    elif rb_diff < 45 or bg_ratio > 0.89:
        undertone = "Cool"
    else:
        undertone = "Neutral"
    ...
    if undertone == "Cool":
        season = "Winter" if (skin_tone in ["Medium", "Deep"] or avg_sat > 40) else "Summer"
    elif undertone == "Warm":
        season = "Autumn" if skin_tone in ["Medium", "Deep"] else "Spring"
    else:
        season = "Summer" if skin_tone == "Fair" else "Autumn"

    sampled_hex = "#%02x%02x%02x" % (int(r_avg), int(g_avg), int(b_avg))
