...
export default function AnalysisExperience({ onScanComplete, theme }) {
  ...
  // Run progress scan for PHOTO mode
  useEffect(() => {
    let interval;
    if (mode === 'photo' && scanState === 'scanning') {
      interval = setInterval(() => {
        setProgress((prev) => {
          const next = prev + 1; // Increments slightly slower for luxury flow
          const currentLog = LOG_STEPS_PHOTO.find(step => step.threshold <= next && step.threshold > prev);
          if (currentLog) {
  ...
  // Run progress scan for MANUAL mode finish
  useEffect(() => {
    let interval;
    if (mode === 'manual' && currentStep === 5) {
      interval = setInterval(() => {
        setProgress((prev) => {
          const next = prev + 1;
          const currentLog = LOG_STEPS_MANUAL.find(step => step.threshold <= next && step.threshold > prev);
          if (currentLog) {
            setScanLog(prevLog => prevLog ? `${prevLog}\n${currentLog.text}` : currentLog.text);
          }
              ...
              <div style={{ 
                ...
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', width: '100%', margin: '0.8rem 0' }}>
                    <div style={{ flex: 1, height: '110px', opacity: 0.85, display: 'flex', alignItems: 'center' }}>
