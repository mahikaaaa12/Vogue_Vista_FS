...
export default function Dashboard({ setScreen }) {
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: '7rem', paddingBottom: '5rem', background: '#0A0A0B', color: '#FAF8F5' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem' }}>
        
        {/* Header Profile Section */}
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1.5rem', marginBottom: '3rem', padding: '2rem', background: 'rgba(18,18,20,0.7)', borderRadius: '1.5rem', border: '1px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(20px)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
            ...
            <div>
              <span style={{ fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.2em', color: '#C6A16A', fontWeight: 600 }}>Atelier Member Profile</span>
              <h1 style={{ fontFamily: "'Cinzel', serif", fontSize: '1.75rem', margin: '0.25rem 0 0 0', fontWeight: 500 }}>{user?.name || 'Vogue Vista Member'}</h1>
              <p style={{ fontSize: '0.85rem', color: '#A1A1AA', margin: 0 }}>{user?.email || 'haute@voguevista.com'}</p>
            </div>
          </div>

          <div style={{ display: 'flex', gap: '1rem' }}>
            <Button variant="secondary" onClick={() => setActiveTab('history')} icon={History}>
              History Logs
            </Button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem', marginBottom: '3rem' }}>
          ...
          <Card hoverable={true} style={{ border: '1px solid rgba(198, 161, 106, 0.3)' }}>
            ...
            <h3 style={{ fontFamily: "'Cinzel', serif", fontSize: '1.4rem', color: '#FAF8F5', margin: '0.5rem 0' }}>Body Analysis</h3>
            ...
            <Button variant="primary" onClick={() => setScreen('body-analysis')} style={{ width: '100%' }}>
              Launch Body Module <ChevronRight size={16} />
            </Button>
          </Card>
          ...
          <Card hoverable={true} style={{ border: '1px solid rgba(56, 189, 248, 0.3)' }}>
            ...
            <h3 style={{ fontFamily: "'Cinzel', serif", fontSize: '1.4rem', color: '#FAF8F5', margin: '0.5rem 0' }}>Color Analysis</h3>
            <p style={{ fontSize: '0.875rem', color: '#A1A1AA', lineHeight: 1.6, marginBottom: '1.5rem' }}>
            ...
            <Button variant="secondary" onClick={() => setScreen('color-analysis')} style={{ width: '100%', border: '1px solid rgba(56, 189, 248, 0.4)', color: '#38BDF8' }}>
              Launch Color Module <ChevronRight size={16} />
            </Button>
          </Card>
        </div>

        {/* Recent Analysis History Table */}
        <div style={{ background: 'rgba(18,18,20,0.7)', borderRadius: '1.5rem', border: '1px solid rgba(255,255,255,0.08)', padding: '2rem', backdropFilter: 'blur(20px)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
            <h3 style={{ fontFamily: "'Cinzel', serif", fontSize: '1.25rem', color: '#FAF8F5', margin: 0 }}>Recent Session History</h3>
            <span style={{ fontSize: '0.8rem', color: '#C6A16A' }}>Telemetry Logged</span>
          </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
                ...
                <tbody>
                  {bodyHistory.map((item, idx) => (
                    <tr key={`body-${idx}`} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      <td style={{ padding: '0.75rem 1rem', color: '#C6A16A', fontWeight: 600 }}>Body Analysis</td>
                      <td style={{ padding: '0.75rem 1rem' }}>{item.predicted_shape || 'Hourglass'}</td>
                      <td style={{ padding: '0.75rem 1rem' }}>{item.confidence ? `${Math.round(item.confidence * 100)}%` : '96%'}</td>
                      <td style={{ padding: '0.75rem 1rem', color: '#A1A1AA' }}>{item.created_at ? new Date(item.created_at).toLocaleDateString() : 'Today'}</td>
                    </tr>
                  ))}
                  {colorHistory.map((item, idx) => (
                    <tr key={`color-${idx}`} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      <td style={{ padding: '0.75rem 1rem', color: '#38BDF8', fontWeight: 600 }}>Color Analysis</td>
                      <td style={{ padding: '0.75rem 1rem' }}>{item.season ? `${item.season} (${item.undertone})` : 'Winter (Cool)'}</td>
                      <td style={{ padding: '0.75rem 1rem' }}>98%</td>
                      <td style={{ padding: '0.75rem 1rem', color: '#A1A1AA' }}>{item.created_at ? new Date(item.created_at).toLocaleDateString() : 'Today'}</td>
                    </tr>
                  ))}
                </tbody>
